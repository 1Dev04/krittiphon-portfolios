...
const SUN_FP: FpTheme = {
  ...
  heroBg: (c) => `${c}12`,
  heroBorder: (c) => `${c}50`,
  heroGlow: (c) => `0 0 24px ${c}18,0 4px 24px rgba(20,60,35,0.10)`,
  heroTitle: "#1a3d28",
  heroSub: (c) => c,
  heroTagBg: (c) => `${c}16`,
  heroTagText: (c) => c,
  ...
  sectionColor: (c) => c,
  sectionLine: (c) => `linear-gradient(90deg,transparent,${c}55)`,
  imgCardBg: "rgba(255,255,255,0.90)",
  imgCardBorder: (c) => `${c}35`,
  imgCardShadow: (c) => `0 8px 40px ${c}18,0 2px 12px rgba(20,60,35,0.09)`,
...
const MOON_FP: FpTheme = {
  ...
  heroBg: (c) => `${c}12`,
  heroBorder: (c) => `${c}55`,
  heroGlow: (c) => `0 0 32px ${c}28,0 0 64px ${c}12`,
  heroTitle: "#e2f8f0",
  heroSub: (c) => c,
  heroTagBg: (c) => `${c}1a`,
  heroTagText: (c) => c,
  ...
  imgCardBorder: (c) => `${c}38`,
  imgCardShadow: (c) => `0 0 56px ${c}22,0 16px 48px rgba(0,0,0,0.55)`,
  imgCaption: "rgba(220,255,240,0.75)",
  imgTag: (c) => c,
  imgTagBg: (c) => `${c}1a`,
...
const ABCSHOP = {
  ...
  months: [
    { month_en:"January",              month_th:"มกราคม",             year:"2026", num:"01", color:"#34d399", glow:"#059669", icon:"🤖",
      tasks:["13–15 Jan — Train AI Model Python + YOLO (Round 1–3) | เทรน AI Model ด้วย Python + YOLO (รอบ 1–3)","16–23 Jan — Design & build Frontend + Backend structure (8 days) | ออกแบบ & ลงมือทำ Structure 8 วัน","24–25 Jan — Update ER Diagram & Planning Structure | อัพเดต ER Diagram & Planning","26 Jan — Mock data Home page | Mock data หน้า Home","27 Jan — Mock data Shop, Flutter folders, split Home sections | Mock data Shop + แยก Section","28 Jan — Gemini AI poster, Mock data Coupon | Generate AI Poster ด้วย Gemini","29 Jan — Mock data Notification, Coupon, History, Order; design Search Page","30–31 Jan — Train AI YOLO Round 4–5 | เทรน YOLO รอบ 4–5"] },
    { month_en:"February (Early)",     month_th:"กุมภาพันธ์ (ต้น)",   year:"2026", num:"02", color:"#38bdf8", glow:"#0284c7", icon:"🔗",
      tasks:["1–7 Feb — Connect Backend all pages + Custom Snackbar + DB structure v1–v4","8 Feb — Build path transfer Backend → callback Frontend","9 Feb — Fix VARCHAR → TEXT to prevent icon corruption","10 Feb — Update 'Learn More' to icon on Notification Page","11 Feb — Fix X button Search Page → return Homepage","12 Feb — Update Frontend V4.2"] },
    { month_en:"February (Late)",      month_th:"กุมภาพันธ์ (ปลาย)", year:"2026", num:"03", color:"#818cf8", glow:"#4f46e5", icon:"🧪",
      tasks:["13 Feb — Flow Diagram, Use Case, update Backend Home + Search","14 Feb — Update Backend Shop, Notification","15 Feb — Meow Size Page, research OpenAI & Jenkins","16 Feb — Meeting with advisor, improve Favourite + Basket","17 Feb — Setup Jenkins Sleep Time Automation","18–22 Feb — Improve Meow Size, Favourite, Basket + dot marker + Sequence Diagram","23 Feb — Fix Alert Text, discover Gemini API","24–28 Feb — Fix Meow Size, add History Page, Deploy Backend, connect Google AI Studio V1–V5"] },
    { month_en:"March (Early)",        month_th:"มีนาคม (ต้น)",       year:"2026", num:"04", color:"#fcd34d", glow:"#d97706", icon:"✨",
      tasks:["1 Mar — Connect Google AI Studio V6","2 Mar — Fix Service API Cat (Frontend), ✅ Success Call Gemini 2.5 Flash","3 Mar — Fix History Edit, Meow Size Recommendation Scan Page","4–5 Mar — Fix History Cat Edit, Meow Size Recommendation Card","6 Mar — Meow Size Recommendation Card V2","7 Mar — Meow Size Recommendation Card V3"] },
    { month_en:"March (Late) — Done!", month_th:"มีนาคม (ปลาย) — สำเร็จ!", year:"2026", num:"05", color:"#fb923c", glow:"#ea580c", icon:"🏁",
      tasks:["8 Mar — Edit Meow Size (Recommend, Edit Cat, Backend CRUD API)","9 Mar — Switch to OpenAI GPT-4.1 mini for Detect & Analysis Cat | เปลี่ยนมาใช้ OpenAI GPT-4.1 mini","10 Mar — Edit Meow Size (Recommend, Edit Cat, History Analysis Cat)","🎉 11 Mar — SUCCESS PROJECT ABCat Shop"] },
  ],
};
  ...
  return (
    <div style={{
      ...
      {(["EN","TH"] as Lang[]).map(l => {
        ...
        return (
          <button key={l} onClick={() => setLang(l)} style={{
            ...
            background: active ? tk.toggleActiveBg(accentColor) : "transparent",
            border: active ? `1px solid ${tk.toggleActiveBorder(accentColor)}` : "1px solid transparent",
            borderRadius: 999, padding: "6px 18px",
            cursor: "pointer",
            boxShadow: active ? tk.toggleActiveShadow(accentColor) : "none",
      ...
      <div style={{
        ...
        left: isEN ? "8px" : "calc(50% + 4px)",
        width: "calc(50% - 12px)", height: 2, borderRadius: 2,
...
function SectionHeader({ icon, label, accentColor, tk }: {
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:32 }}>
      ...
      <div style={{ flex:1, height:1, background:`linear-gradient(90deg,${accentColor}55,transparent)` }} />
    </div>
  );
}
...
function TimelineCard({ item, isMobile, visible, lang, tk }: {
  item: typeof ABCSHOP.months[0]; isMobile: boolean; visible: boolean; lang: Lang; tk: FpTheme;
}) {
    ...
    >
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:10, fontFamily:"'Space Mono',monospace", color:tk.cardYear, letterSpacing:".2em" }}>
            {item.year}
          </div>
        </div>
        <div style={{
...
function TimelineItem({ item, index, isMobile, isLast, lang, tk }: {
  item: typeof ABCSHOP.months[0]; index: number; isMobile: boolean; isLast: boolean; lang: Lang; tk: FpTheme;
}) {
    ...
    <>
      ...
      <section id="projects-reference" style={{
        ...
        {([
          ...
          <div key={i} style={{ position:"absolute", width:o.w, height:o.h, top:o.t, left:o.l, right:o.r, borderRadius:"50%", filter:"blur(88px)", background:o.bg, animation:`fpOrbDrift ${o.d} ease-in-out ${o.dl} infinite`, pointerEvents:"none", transition:"background .5s" }} />
        ))}

        {/* Scanline */}
        <div style={{ position:"absolute", left:0, right:0, height:2, background:`linear-gradient(90deg,transparent,${tk.scanLine},transparent)`, animation:"fpScan 10s linear infinite", pointerEvents:"none", zIndex:2, opacity:isDark?1:.5 }} />

        {/* Particles — dark only */}
        {isDark && Array.from({ length:18 }, (_,i) => {
          const colors = ["#34d399","#38bdf8","#a78bfa","#fcd34d","#fb923c","#f0abfc"];
          const c = colors[i%6];
          return <div key={i} style={{ position:"absolute", left:`${(i*47.3)%100}%`, top:`${(i*31.7)%100}%`, width:(i%3)+1, height:(i%3)+1, borderRadius:"50%", background:c, boxShadow:`0 0 ${(i%3+2)*3}px ${c}`, animation:`fpOrbDrift ${5+i%5}s ease-in-out ${(i*.3)%5}s infinite`, pointerEvents:"none", opacity:.62 }} />;
        })}

        {/* ── Content ── */}
        <div style={{ position:"relative", zIndex:3, maxWidth:1100, margin:"0 auto", padding: isMobile?"0 16px":"0 clamp(20px,5%,60px)", opacity:switching?.4:1, transition:"opacity .22s" }}>
          ...
          <div ref={headerRef} style={{ textAlign:"center", marginBottom:isMobile?40:56, opacity:headerVisible?1:0, transform:headerVisible?"none":"translateY(-24px)", transition:"opacity .8s ease,transform .8s cubic-bezier(.22,.68,0,1.2)" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:14, marginBottom:14 }}>
              ...
              <span style={{ fontSize:10, color:tk.eyebrowColor, letterSpacing:".40em", fontFamily:"'Space Mono',monospace", transition:"color .5s" }}>
                ✦ FEATURED PROJECT ✦
              </span>
            ...
            <h1 style={{
              ...
              color: tk.headingColor,
              textShadow: tk.headingShadow,
              lineHeight:1.1, marginBottom:8,
              transition:"color .5s,text-shadow .5s,font-family .4s",
            }}>ABCat Shop</h1>
            <p style={{ fontFamily:"'Space Mono',monospace", fontSize:11, color:tk.eyebrowColor, letterSpacing:".18em", textTransform:"uppercase", transition:"color .5s" }}>
              AI-Powered eCommerce · Smart Sizing Feature
            </p>
          </div>

          {/* ══ HERO BADGE ══ */}
          <div style={{ display:"flex", justifyContent:"center", marginBottom:isMobile?36:48, animation:"fpFadeUp .5s ease both" }}>
            <div style={{
              ...
              <div>
                <div style={{ fontFamily:"'Cinzel',serif", fontSize:isMobile?13:15, color:tk.heroTitle, fontWeight:700, letterSpacing:".06em", transition:"color .5s" }}>
                  {project.name}
                </div>
                <div style={{ fontFamily:"'Space Mono',monospace", fontSize:10, color:tk.heroSub(project.accentColor), letterSpacing:".12em" }}>
                  {t(project.role_en, project.role_th)}
                </div>
              </div>
              <div style={{ padding:"4px 14px", borderRadius:999, background:tk.heroTagBg(project.accentColor), border:`1px solid ${tk.heroBorder(project.accentColor)}`, fontSize:10, color:tk.heroTagText(project.accentColor), fontFamily:"'Space Mono',monospace", letterSpacing:".1em", fontWeight:700 }}>
                {t(project.tag_en, project.tag_th)}
              </div>
              <div style={{ padding:"4px 14px", borderRadius:999, background:tk.heroTagBg(project.accentColor), border:`1px solid ${tk.heroBorder(project.accentColor)}`, fontSize:10, color:tk.heroTagText(project.accentColor), fontFamily:"'Space Mono',monospace", letterSpacing:".1em" }}>
                {t(project.period_en, project.period_th)}
              </div>
              {/* YouTube */}
              <a href={project.youtubeUrl} target="_blank" rel="noopener noreferrer"
                style={{ display:"flex", alignItems:"center", gap:6, padding:"6px 16px", borderRadius:999, background:"rgba(255,0,0,0.12)", border:"1.5px solid rgba(255,0,0,0.35)", textDecoration:"none", cursor:"pointer", transition:"all .25s ease" }}
                onMouseEnter={e => { const el = e.currentTarget as HTMLAnchorElement; el.style.background="rgba(255,0,0,0.22)"; el.style.borderColor="rgba(255,0,0,0.60)"; el.style.transform="scale(1.05)"; }}
                onMouseLeave={e => { const el = e.currentTarget as HTMLAnchorElement; el.style.background="rgba(255,0,0,0.12)"; el.style.borderColor="rgba(255,0,0,0.35)"; el.style.transform="scale(1)"; }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="#ff0000"><path d="M23.5 6.2s-.23-1.65-.96-2.38c-.91-.96-1.94-.96-2.4-1.02C17.14 2.6 12 2.6 12 2.6s-5.14 0-8.14.2c-.47.06-1.49.06-2.4 1.02C.73 4.55.5 6.2.5 6.2S.27 8.13.27 10.05v1.8c0 1.92.23 3.85.23 3.85s.23 1.65.96 2.38c.91.96 2.12.93 2.65 1.03C5.73 19.3 12 19.3 12 19.3s5.14 0 8.14-.2c.47-.06 1.49-.06 2.4-1.02.73-.73.96-2.38.96-2.38s.23-1.93.23-3.85v-1.8C23.73 8.13 23.5 6.2 23.5 6.2zM9.73 14.59V8.41l6.55 3.1-6.55 3.08z" /></svg>
                <span style={{ fontFamily:"'Space Mono',monospace", fontSize:10, color:"#ff4444", fontWeight:700, letterSpacing:".1em", whiteSpace:"nowrap" }}>VIDEO</span>
              </a>
            </div>
          </div>
          ...
          <div style={{
            ...
            background:tk.imgCardBg, border:`1.5px solid ${tk.imgCardBorder(project.accentColor)}`,
            backdropFilter:"blur(14px)", animation:"fpFadeUp .6s .1s ease both",
          }}>
            <p style={{ fontFamily:"'Sarabun',sans-serif", fontSize:isMobile?13:14, lineHeight:1.9, color:tk.imgCaption, textAlign:"center", letterSpacing:".01em", transition:"color .5s" }}>
              {t(project.desc_en, project.desc_th)}
            </p>
          </div>

          {/* ══ STATS ══ */}
          <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"repeat(3,1fr)", gap:isMobile?12:20, marginBottom:isMobile?52:72 }}>
            {project.stats.map((s,i) => (
              <div key={i} style={{
                textAlign:"center", padding:isMobile?"20px 14px":"26px 18px",
                borderRadius:18, background:tk.statBg,
                border:`1.5px solid ${tk.statBorder(project.accentColor)}`,
                backdropFilter:"blur(14px)", position:"relative", overflow:"hidden",
                animation:`fpFadeUp .5s ${i*.1}s ease both`, transition:"background .5s,border-color .5s",
              }}>
                ...
                <div style={{ fontFamily:"'Cinzel',serif", fontSize:13, color:tk.statLabel(project.accentColor), fontWeight:700, letterSpacing:".08em", marginBottom:4, transition:"color .5s" }}>
                  {t(s.label_en, s.label_th)}
                </div>
                <div style={{ fontFamily:"'Sarabun',sans-serif", fontSize:12, color:tk.statSub, transition:"color .5s" }}>
                  {t(s.sub_en, s.sub_th)}
                </div>
              </div>
            ))}
          ...
          {/* ══ PLANNING STRUCTURE ══ */}
          <div ref={planRef}>
            <SectionHeader icon="📐" label={t("PLANNING STRUCTURE","โครงสร้างการวางแผน")} accentColor={project.accentColor} tk={tk} />
            <div style={{
              borderRadius:20, overflow:"hidden",
              border:`1.5px solid ${tk.imgCardBorder(project.accentColor)}`,
              boxShadow:tk.imgCardShadow(project.accentColor), background:tk.imgCardBg,
              opacity:planVisible?1:0, transform:planVisible?"none":"translateY(20px)",
              transition:"opacity .7s ease,transform .7s cubic-bezier(.22,.68,0,1.2)", backdropFilter:"blur(12px)",
            }}>
              <div style={{ padding:"12px 20px", borderBottom:`1px solid ${tk.imgCardBorder(project.accentColor)}`, display:"flex", alignItems:"center", gap:10, background:isDark?"rgba(0,0,0,0.25)":"rgba(255,255,255,0.60)" }}>
                <div style={{ display:"flex", gap:6 }}>
                  {["#ef4444","#fbbf24","#34d399"].map((c,i) => <div key={i} style={{ width:10, height:10, borderRadius:"50%", background:c }} />)}
                </div>
                <span style={{ fontFamily:"'Space Mono',monospace", fontSize:10, color:tk.sectionColor(project.accentColor), letterSpacing:".14em", fontWeight:700, transition:"color .5s" }}>
                  ABCat Shop — {t("Planning Structure & AI Smart Sizing","โครงสร้างการวางแผน & AI Smart Sizing")}
                </span>
                <div style={{ marginLeft:"auto" }}>
                  <span style={{ padding:"2px 10px", borderRadius:999, background:tk.imgTagBg(project.accentColor), border:`1px solid ${tk.imgCardBorder(project.accentColor)}`, fontSize:9, color:tk.imgTag(project.accentColor), fontFamily:"'Space Mono',monospace", letterSpacing:".16em", fontWeight:700 }}>ARCHITECTURE</span>
                </div>
              </div>
              <div style={{ position:"relative", backgroundColor:isDark?"#0a0814":"#f8f9f8" }}>
                {!imgLoaded && (
                  <div style={{ height:300, display:"flex", alignItems:"center", justifyContent:"center", color:tk.sectionColor(project.accentColor), fontFamily:"'Space Mono',monospace", fontSize:11, letterSpacing:".14em", gap:8 }}>
                ...
                <img src={project.planningImg} alt="ABCat Shop Planning Structure" onLoad={() => setImgLoaded(true)}
                  style={{ width:"100%", display:"block", objectFit:"contain", opacity:imgLoaded?1:0, transition:"opacity .5s ease", animation:imgLoaded?"fpImgIn .6s ease both":"none" }} />
              </div>
              <div style={{ padding:"16px 24px", borderTop:`1px solid ${tk.imgCardBorder(project.accentColor)}`, background:isDark?"rgba(0,0,0,0.30)":"rgba(255,255,255,0.70)" }}>
                <p style={{ fontFamily:"'Sarabun',sans-serif", fontSize:12, color:tk.imgCaption, lineHeight:1.7, margin:0, transition:"color .5s" }}>
                  <strong style={{ color:tk.sectionColor(project.accentColor), fontWeight:700 }}>
                    {t("Architecture Overview:","สถาปัตยกรรมระบบ:")}
                  </strong>{" "}
                  {t(
                    "ABCat Shop: Flutter (Frontend) · FastAPI + Python (Backend & AI) · PostgreSQL + Cloudinary (DB & Storage) · Firebase (Auth) · Jenkins (CI/CD) · Render (Deploy) · OpenAI GPT-4.1 mini + Gemini 2.5 Flash for cat detection & analysis.",
                    "ระบบ ABCat Shop ประกอบด้วย Flutter · FastAPI + Python · PostgreSQL + Cloudinary · Firebase · Jenkins + Render และ OpenAI GPT-4.1 mini + Gemini 2.5 Flash สำหรับ Detect & Analysis แมว"
                  )}
                </p>
              </div>
            </div>
          ...
          <div style={{ marginTop:isMobile?52:76 }}>
            <SectionHeader icon="🗓" label={t(`TIMELINE · ${project.name}`,`ไทม์ไลน์ · ${project.name}`)} accentColor={project.accentColor} tk={tk} />
            <div style={{ position:"relative" }}>
              {!isMobile && (
                <div style={{ position:"absolute", left:"50%", top:28, bottom:28, width:2, transform:"translateX(-50%)", background:`linear-gradient(180deg,${project.months.map(m=>m.color+"55").join(",")})`, animation:"fpLineGrow 1.2s ease both", zIndex:1 }} />
              )}
          ...
          <div ref={techRef} style={{ marginTop:isMobile?52:76 }}>
            <SectionHeader icon="⚙" label={t("TECH STACK","เทคโนโลยีที่ใช้")} accentColor={project.accentColor} tk={tk} />
            <div style={{ display:"flex", flexWrap:"wrap", gap:isMobile?8:12, justifyContent:"center" }}>
              {project.tech.map((t2,i) => (
                <div key={i} style={{
                  padding:isMobile?"7px 13px":"9px 17px", borderRadius:999,
                  background:tk.techBg(t2.color), border:`1.5px solid ${tk.techBorder(t2.color)}`,
                  ...
                  <div style={{ width:6, height:6, borderRadius:"50%", background:t2.color, boxShadow:`0 0 8px ${t2.color}`, flexShrink:0 }} />
                  <span style={{ fontFamily:"'Space Mono',monospace", fontSize:isMobile?10:11, color:tk.techLabel(t2.color), fontWeight:700, letterSpacing:".08em" }}>{t2.label}</span>
                  <span style={{ fontSize:9, color:tk.techCat, fontFamily:"'Space Mono',monospace", letterSpacing:".1em", textTransform:"uppercase", transition:"color .5s" }}>{t2.cat}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display:"flex", justifyContent:"center", marginTop:isMobile?48:64 }}>
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:12 }}>
              <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                ...
                <span style={{ fontFamily:"'Space Mono',monospace", fontSize:9, letterSpacing:".22em", color:isDark?"rgba(52,211,153,0.38)":"rgba(45,106,79,0.35)" }}>LANGUAGE</span>
              ...
              <p style={{ fontFamily:"'Space Mono',monospace", fontSize:9, letterSpacing:".16em", color:isDark?"rgba(52,211,153,0.32)":"rgba(45,106,79,0.30)", margin:0 }}>
