...
interface ThemeTk {
  ...
  pageHeading: string;
  pageHeadingFont: string;
  pageHeadingColor: string;
  pageHeadingShadow: string;
  sectionLabel: string;
...
const SUN: ThemeTk = {
  ...
  pageHeading: "#1a3d28",
  pageHeadingFont: "'Lora',serif",
  pageHeadingColor: "#1a3d28",
  pageHeadingShadow: "0 1px 0 rgba(255,255,255,0.9),0 2px 12px rgba(45,106,79,0.15)",
  sectionLabel: "#2d6a4f",
  ...
  awardYearColor: "rgba(255,255,255,0.65)",
  popupBg: "rgba(230,245,235,0.97)",
  popupBorder: (c) => `${c}38`,
  ...
  cardBorderHov: (c) => `${c}44`,
  cardShadow: "0 2px 12px rgba(20,60,35,0.07),0 1px 3px rgba(20,60,35,0.04)",
  cardShadowHov: (c) => `0 8px 32px ${c}20,0 2px 8px rgba(20,60,35,0.09)`,
  cardImgOverlay: "linear-gradient(to top,rgba(10,30,15,0.75) 0%,transparent 55%)",
  cardTitle: "#1a3d28",
  cardTitleHov: "#0d2618",
  cardDesc: "rgba(20,50,30,0.58)",
  cardMeta: "rgba(20,50,30,0.38)",
  cardHint: "rgba(20,50,30,0.26)",
  cardHintHov: (c) => c,
  modalBackdrop: "rgba(10,30,15,0.78)",
  modalBg: "rgba(245,252,248,0.99)",
  modalBorder: (c) => `${c}40`,
  modalShadow: (c) => `0 24px 80px ${c}20,0 8px 32px rgba(10,30,15,0.14)`,
  modalSpine: (c) => `linear-gradient(180deg,transparent,${c}33,transparent)`,
  modalLine: "rgba(45,106,79,0.05)",
  modalSealColor: "#ffffff",
  modalCorner: (c) => `${c}44`,
  modalHeading: (c) => c,
  modalDesc: "rgba(20,50,30,0.62)",
  modalFooter: "rgba(20,50,30,0.28)",
...
const MOON: ThemeTk = {
  ...
  orbC: "rgba(236,72,153,0.10)",
  scanLine: "rgba(167,139,250,0.18)",
  particles: ["#a78bfa","#67e8f9","#86efac","#fda4af","#fcd34d"],
  bottomFade: "linear-gradient(transparent,#05050e)",
  pageEyebrow: "#a78bfa",
  pageEyebrowLine: "rgba(167,139,250,0.42)",
  pageHeading: "#f1f5f9",
  pageHeadingFont: "'Cinzel',serif",
  pageHeadingColor: "#e9d5ff",
  pageHeadingShadow: "0 0 32px rgba(167,139,250,0.55),0 2px 8px rgba(0,0,0,0.5)",
  sectionLabel: "#fcd34d",
  sectionLabelLine: "rgba(252,211,77,0.38)",
  dividerLine: "rgba(167,139,250,0.22)",
  dividerText: "rgba(167,139,250,0.48)",
  awardSlideBorder: (c) => `${c}44`,
  awardSlideGlow: (c) => `0 4px 60px ${c}28,0 20px 60px rgba(0,0,0,0.58)`,
  awardOverlay: "linear-gradient(to top,rgba(5,5,14,0.94) 0%,rgba(5,5,14,0.35) 50%,transparent 100%)",
  awardTagBg: (c) => `${c}1e`,
  awardTagBorder: (c) => `${c}55`,
  awardYearColor: "rgba(255,255,255,0.48)",
  popupBg: "rgba(4,4,20,0.96)",
  popupBorder: (c) => `${c}44`,
  popupBodyBg: "rgba(6,5,22,0.99)",
  tabActiveBg: (c) => `linear-gradient(135deg,${c}22,${c}10)`,
  tabActiveBorder: (c) => `${c}77`,
  tabActiveGlow: (c) => `0 0 18px ${c}2e`,
  ...
  cardShadowHov: (c) => `0 0 32px ${c}22,0 12px 40px rgba(0,0,0,0.60)`,
  cardImgOverlay: "linear-gradient(to top,rgba(5,5,14,0.90) 0%,transparent 55%)",
  cardTitle: "#e2d9f3",
  cardTitleHov: "#f1f5f9",
  cardDesc: "rgba(200,185,255,0.52)",
  cardMeta: "rgba(200,185,255,0.34)",
  cardHint: "rgba(167,139,250,0.30)",
  cardHintHov: (c) => c,
  modalBackdrop: "rgba(2,1,12,0.90)",
  modalBg: "rgba(8,6,24,0.98)",
  modalBorder: (c) => `${c}44`,
  modalShadow: (c) => `0 0 80px ${c}28,0 30px 80px rgba(0,0,0,0.80)`,
  modalSpine: (c) => `linear-gradient(180deg,transparent,${c}44,transparent)`,
  modalLine: "rgba(167,139,250,0.028)",
  modalSealColor: "#ffffff",
  modalCorner: (c) => `${c}55`,
  modalHeading: (c) => c,
  modalDesc: "rgba(220,210,255,0.62)",
  modalFooter: "rgba(167,139,250,0.25)",
...
const certs: Record<Tab, Cert[]> = {
  ...
  backend: [
    ...
    { image:"https://res.cloudinary.com/dag73dhpl/image/upload/v1773285803/Java_g0imtf.jpg",     title:"Java Programming",desc:"Java with Born to Dev. Built CheeseCake_Cafe.",     tag:"BACKEND", color:"#a78bfa" },
    { image:"https://res.cloudinary.com/dag73dhpl/image/upload/v1773285794/C_uxveew.jpg",        title:"C Programming",  desc:"Zero To One: C Programming — Built CakeCafe_C.",   tag:"BACKEND", color:"#a78bfa" },
  ],
  other: [
...
/* ══════════════════════════════════════════════════════════════════
   SCROLL LOCK HOOK — ล็อค scroll ด้วย overflow:hidden บน html
══════════════════════════════════════════════════════════════════ */
function useScrollLock(locked: boolean) {
  useEffect(() => {
    ...
    // ครอบ Next.js #__next wrapper ด้วย เพราะมันอาจเป็น scroll container จริง
    const nextRoot = document.getElementById("__next") as HTMLElement | null;
    const scrollbarWidth = window.innerWidth - html.clientWidth;

    // snapshot ค่าเดิมก่อน lock
    type Snap = { el: HTMLElement; overflow: string; overflowY: string };
    const targets = ([html, body, nextRoot] as (HTMLElement | null)[]).filter(Boolean) as HTMLElement[];
    const snap: Snap[] = targets.map(el => ({
      el,
    ...
    return () => {
      snap.forEach(({ el, overflow, overflowY }) => {
...
function CertModal({ cert, isOpen, onClose, isMobile, tk }: {
  ...
  // ✅ SCROLL LOCK — ป้องกัน scroll page ด้านหลัง
  ...
  // ✅ Portal — render ออกนอก DOM tree ของ section ทั้งหมด
  return createPortal(
    // ✅ z-index สูงพอ ครอบ navbar ทุกอย่าง
    <div
      onClick={onClose}
      style={{
        position: "fixed",
    ...
    >
      <div
        ...
        style={{
          ...
          margin: "auto",
          animation: isOpen ? "acBookOpen .5s cubic-bezier(.22,.68,0,1.2) both" : "none",
        }}
      >
        {/* glow under */}
        <div style={{
          position: "absolute", bottom: -20, left: "12%", right: "12%", height: 20,
          background: `radial-gradient(ellipse,${cert.color}28 0%,transparent 70%)`,
        ...
        <div style={{
          ...
          <div style={{ position:"absolute", top:0, left:32, bottom:0, width:1, background:tk.modalSpine(cert.color), zIndex:1, pointerEvents:"none" }} />
          {Array.from({ length: 6 }, (_, i) => (
            <div key={i} style={{ position:"absolute", left:0, right:0, height:1, top:`${14+i*13}%`, background:tk.modalLine, pointerEvents:"none" }} />
          ))}
          <div style={{
          ...
          {hasImg && (
            <div style={{
              ...
              <div style={{ position:"absolute", inset:0, background:!isMobile?`linear-gradient(90deg,transparent 65%,rgba(0,0,0,.55) 100%)`:`linear-gradient(0deg,rgba(0,0,0,.7) 0%,transparent 55%)`, pointerEvents:"none" }} />
              <span style={{
                position:"absolute", top:10, left:10, padding:"2px 10px", borderRadius:999,
                background:`${cert.color}1e`, border:`1px solid ${cert.color}55`,
                backdropFilter:"blur(8px)", fontSize:10, color:cert.color,
          ...
          <div style={{
            ...
            <p style={{ margin:"0 0 12px", textAlign:"center", fontFamily:"'Space Mono',monospace", fontSize:10, color:`${cert.color}88`, letterSpacing:".38em", textTransform:"uppercase" }}>
            ...
            {!hasImg && (
              <div style={{ display:"flex", justifyContent:"center", marginBottom:14 }}>
                <span style={{ padding:"3px 14px", borderRadius:999, background:`${cert.color}14`, border:`1px solid ${cert.color}44`, fontSize:10, color:cert.color, fontFamily:"'Space Mono',monospace", letterSpacing:".18em", fontWeight:700 }}>{cert.tag}</span>
              </div>
            )}
            <p style={{ margin:"0 0 20px", fontSize:isMobile?13:14, fontFamily:"Georgia,serif", fontStyle:"italic", color:tk.modalDesc, lineHeight:1.85, textAlign:"center" }}>
              {cert.desc}
            </p>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}>
              <div style={{ flex:1, height:1, background:`linear-gradient(90deg,transparent,${cert.color}22)` }} />
              <div style={{ width:34, height:34, borderRadius:"50%", border:`1px solid ${cert.color}44`, background:`${cert.color}0e`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, color:`${cert.color}bb` }}>ᚠ</div>
              <div style={{ flex:1, height:1, background:`linear-gradient(90deg,${cert.color}22,transparent)` }} />
            </div>
            <p style={{ margin:"8px 0 0", textAlign:"center", fontSize:10, color:tk.modalFooter, fontFamily:"'Space Mono',monospace", letterSpacing:".16em" }}>
              1DEV · KNOWLEDGE CODEX
            </p>
          </div>
        </div>
    ...
    document.body   // ← Portal target: render ตรง body หลุดพ้น stacking context ทั้งหมด
  );
}
...
function CertCard({ cert, idx, onOpen, isMobile, tk }: {
  ...
  return (
    ...
    >
      ...
      {/* body */}
      <div style={{ padding:hasImg?"14px 16px 16px":"20px 18px 18px", display:"flex", flexDirection:"column", flex:1, position:"relative" }}>
        {/* tag */}
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:9 }}>
          <span style={{
            ...
            <span style={{ width:4, height:4, borderRadius:"50%", background:cert.color, boxShadow:`0 0 4px ${cert.color}`, display:"inline-block" }} />
            {cert.tag}
          </span>
        </div>
        {/* title */}
        ...
        {/* desc */}
        <p style={{ margin:"0 0 14px", fontSize:12.5, lineHeight:1.65, flex:1, color:tk.cardDesc, fontFamily:"Georgia,serif", fontStyle:"italic" }}>
          {cert.desc}
        </p>
        {/* footer */}
        <div style={{
          ...
          <span style={{ fontSize:12, color: h ? cert.color : tk.cardMeta, fontFamily:"'Space Mono',monospace", opacity:h?1:.6, transition:"all .25s" }}>→</span>
        </div>
      </div>
    </div>
  );
...
function AwardSlideshow({ isMobile, tk, isDark }: { isMobile: boolean; tk: ThemeTk; isDark: boolean }) {
  ...
  // ✅ scroll lock for award popup too
  useScrollLock(popup !== null);

  const go = useCallback((n: number, d: "next"|"prev" = "next") => {
    setDir(d); setCur((n + awards.length) % awards.length);
  }, []);

  useEffect(() => {
    if (paused || popup !== null) return;
  ...
  return (
    <div style={{ width:"100%", maxWidth:920, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:28 }}>
        ...
        <span style={{ fontFamily:"'Cinzel',serif", fontSize:12, color:tk.sectionLabel, letterSpacing:".32em", textTransform:"uppercase", fontWeight:700 }}>Awards & Achievements</span>
      ...
      >
        <img key={cur} src={award.img} alt={award.title} style={{
        ...
        {/* particles on slide — dark only */}
        {isDark && Array.from({ length:6 }, (_,i) => (
          <div key={i} style={{ position:"absolute", left:`${15+i*14}%`, top:`${10+i*8}%`, width:(i%2)+1, height:(i%2)+1, borderRadius:"50%", background:award.color, boxShadow:`0 0 6px ${award.color}`, animation:`acFloat ${3+i*.5}s ease-in-out ${i*.3}s infinite`, pointerEvents:"none", opacity:.5 }} />
        ))}

        {/* corner decorations */}
        {[["top","left"],["top","right"],["bottom","right"]].map(([v,h],ki)=>(
          <div key={ki} style={{ position:"absolute", zIndex:2, pointerEvents:"none", top:v==="top"?16:"auto", bottom:v==="bottom"?16:"auto", left:h==="left"?16:"auto", right:h==="right"?16:"auto", width:20, height:20, borderTop:v==="top"?`1.5px solid ${award.color}77`:"none", borderBottom:v==="bottom"?`1.5px solid ${award.color}77`:"none", borderLeft:h==="left"?`1.5px solid ${award.color}77`:"none", borderRight:h==="right"?`1.5px solid ${award.color}77`:"none" }} />
        ))}

        {/* expand hint */}
        <div style={{ position:"absolute", top:16, right:16, width:34, height:34, borderRadius:10, background:"rgba(0,0,0,.45)", backdropFilter:"blur(8px)", border:`1px solid ${award.color}44`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, color:award.color, pointerEvents:"none" }}>⊕</div>

        {/* text block */}
        <div style={{ position:"absolute", bottom:0, left:0, right:0, padding:isMobile?"20px 20px 22px":"32px 36px 36px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10, flexWrap:"wrap" }}>
            <span style={{ padding:"3px 10px", borderRadius:999, background:tk.awardTagBg(award.color), border:`1px solid ${tk.awardTagBorder(award.color)}`, backdropFilter:"blur(8px)", fontSize:11, color:award.color, fontFamily:"'Space Mono',monospace", letterSpacing:".14em", fontWeight:700 }}>{award.org}</span>
            <span style={{ fontSize:11, color:tk.awardYearColor, fontFamily:"'Space Mono',monospace" }}>{award.year}</span>
          </div>
          <h3 style={{ margin:0, fontFamily:"'Cinzel',serif", fontSize:isMobile?"clamp(17px,4.5vw,24px)":"clamp(22px,2.2vw,30px)", fontWeight:700, color:"#ffffff", letterSpacing:".04em", lineHeight:1.3, textShadow:`0 2px 16px rgba(0,0,0,.85),0 0 24px ${award.color}44` }}>{award.title}</h3>
        </div>

        {/* progress bar */}
        {!paused && popup === null && (
          <div style={{ position:"absolute", bottom:0, left:0, height:3, background:award.color, animation:"acProgress 3.8s linear forwards", boxShadow:`0 0 10px ${award.color}`, zIndex:2 }} />
        )}
        ...
        {["←","→"].map((arr,ai)=>(
          <button key={arr}
            ...
            style={{ position:"absolute", top:"50%", transform:"translateY(-50%)", [ai===0?"left":"right"]:16, width:40, height:40, borderRadius:12, background:"rgba(0,0,0,.50)", backdropFilter:"blur(10px)", border:"1px solid rgba(255,255,255,.16)", color:"rgba(255,255,255,.88)", fontSize:14, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", transition:"all .2s", zIndex:3 }}>{arr}</button>
        ))}
      </div>
      ...
      <div style={{ display:"flex", gap:8, justifyContent:"center", marginTop:20 }}>
        {awards.map((a,i)=>(
          <button key={i} onClick={() => { setDir("next"); setCur(i); setPaused(true); }}
            style={{ width:i===cur?32:8, height:8, borderRadius:999, padding:0, border:"none", cursor:"pointer", background:i===cur?a.color:"rgba(128,128,128,0.30)", boxShadow:i===cur?`0 0 10px ${a.color}99`:"none", transition:"all .38s cubic-bezier(.22,.68,0,1.2)" }} />
        ))}
      </div>

      {/* award popup — Portal ออกนอก DOM tree */}
      {popup !== null && createPortal(
        <div
          onClick={() => { setPopup(null); setPaused(false); }}
          style={{ position:"fixed", top:0, left:0, right:0, bottom:0, zIndex:99998, background:tk.popupBg, backdropFilter:"blur(20px)", WebkitBackdropFilter:"blur(20px)", display:"flex", alignItems:"center", justifyContent:"center", padding:20, animation:"acFadeIn .25s ease both" }}
        >
          <div onClick={e => e.stopPropagation()} style={{ position:"relative", maxWidth:880, width:"100%", borderRadius:18, overflow:"hidden", border:`1.5px solid ${tk.popupBorder(awards[popup].color)}`, boxShadow:`0 0 80px ${awards[popup].color}22,0 30px 80px rgba(0,0,0,.55)`, background:tk.popupBodyBg, animation:"acPopIn .3s cubic-bezier(.22,.68,0,1.2) both" }}>
            <img src={awards[popup].img} alt={awards[popup].title} style={{ width:"100%", display:"block", maxHeight:"80vh", objectFit:"contain" }} />
            <div style={{ position:"absolute", bottom:0, left:0, right:0, padding:"18px 24px", background:"linear-gradient(0deg,rgba(0,0,0,.90) 0%,transparent 100%)" }}>
              <h3 style={{ margin:0, fontFamily:"'Cinzel',serif", fontSize:18, color:"#fff", fontWeight:700, textShadow:"0 2px 12px rgba(0,0,0,.9)" }}>{awards[popup].title}</h3>
              <p style={{ margin:"4px 0 0", fontSize:13, color:awards[popup].color, fontFamily:"'Space Mono',monospace" }}>{awards[popup].org} · {awards[popup].year}</p>
            </div>
            <button onClick={() => { setPopup(null); setPaused(false); }} style={{ position:"absolute", top:12, right:12, width:30, height:30, borderRadius:"50%", background:"rgba(0,0,0,.7)", border:"1px solid rgba(255,255,255,.2)", color:"#fff", fontSize:14, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>✕</button>
            {["←","→"].map((arr,ai)=>(
              <button key={arr} onClick={()=>setPopup((popup+(ai===0?-1:1)+awards.length)%awards.length)} style={{ position:"absolute", top:"50%", transform:"translateY(-50%)", [ai===0?"left":"right"]:12, width:38, height:38, borderRadius:10, background:"rgba(0,0,0,.65)", border:"1px solid rgba(255,255,255,.15)", color:"#fff", fontSize:14, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>{arr}</button>
            ))}
          </div>
        </div>,
        document.body
...
function CertSection({ isMobile, tk }: { isMobile: boolean; tk: ThemeTk }) {
  ...
  const openModal = (c: Cert) => {
    setCert(c);
    // small delay so DOM mounts before animation
    requestAnimationFrame(() => requestAnimationFrame(() => setIsOpen(true)));
  };
  const closeModal = () => {
    setIsOpen(false);
    setTimeout(() => setCert(null), 350);
  };
  ...
  return (
    <div style={{ width:"100%", maxWidth:960, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:28 }}>
        <span style={{ fontSize:20 }}>📜</span>
        <span style={{ fontFamily:"'Cinzel',serif", fontSize:12, color:tk.sectionLabel, letterSpacing:".32em", textTransform:"uppercase", fontWeight:700 }}>Certificates</span>
        <div style={{ flex:1, height:1, background:`linear-gradient(90deg,${tk.sectionLabelLine},transparent)` }} />
      </div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:28, flexWrap:"wrap" }}>
        {TABS.map(t => {
          ...
          return (
            <button key={t.key} onClick={() => setTab(t.key)} style={{
              ...
              padding:isMobile?"7px 14px":"8px 22px", borderRadius:40, cursor:"pointer", outline:"none",
              ...
              fontFamily:"'Cinzel',serif", fontSize:isMobile?11:13, letterSpacing:".08em",
        ...
        <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:6, fontSize:11, color:tk.pageEyebrow, fontFamily:"'Space Mono',monospace", letterSpacing:".12em", opacity:.6 }}>
          {certs[tab].length} records
        </div>
      </div>

      {/* grid */}
      <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"repeat(3,1fr)", gap:isMobile?14:16 }}>
        {certs[tab].map((c, i) => (
          <CertCard key={`${tab}-${i}`} cert={c} idx={i} onOpen={openModal} isMobile={isMobile} tk={tk} />
        ))}
      </div>
...
export default function AwardAndCertificatePage() {
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Cinzel:wght@400;700;900&family=Space+Mono:wght@400;700&family=Lora:ital,wght@0,600;1,600&display=swap');
        ...
        @keyframes acScan      { 0%{transform:translateY(-100vh)} 100%{transform:translateY(100vh)} }
      ...
      <section id="award-certificates" style={{
        ...
        {/* Grid */}
        <div style={{ position:"absolute", inset:0, pointerEvents:"none", backgroundImage:`linear-gradient(${tk.gridLine} 1px,transparent 1px),linear-gradient(90deg,${tk.gridLine} 1px,transparent 1px)`, backgroundSize:"60px 60px" }} />
        ...
        {([
          ...
          <div key={i} style={{ position:"absolute", width:o.w, height:o.h, top:o.t, right:o.r, bottom:o.b, left:o.l, borderRadius:"50%", filter:"blur(72px)", background:o.bg, animation:`acOrb ${o.d} ease-in-out ${o.dl} infinite`, pointerEvents:"none", transition:"background .5s" }} />
        ))}

        {/* Scanline — transform แทน top ป้องกันกระตุก */}
        <div style={{ position:"absolute", top:0, left:0, right:0, height:1.5, background:`linear-gradient(90deg,transparent,${tk.scanLine},transparent)`, animation:"acScan 12s linear infinite", pointerEvents:"none", zIndex:2, opacity:isDark?1:.5, willChange:"transform" }} />

        {/* Particles — dark only */}
        {isDark && Array.from({ length:20 }, (_,i) => (
          <div key={i} style={{ position:"absolute", left:`${(i*47.3+9)%100}%`, top:`${(i*33.1+5)%100}%`, width:(i%3)+1, height:(i%3)+1, borderRadius:"50%", background:tk.particles[i%5], boxShadow:`0 0 ${(i%3+1)*4}px ${tk.particles[i%5]}`, animation:`acFloat ${4+i%5}s ease-in-out ${(i*.3)%4}s infinite`, pointerEvents:"none", opacity:.65 }} />
        ))}
        ...
        <div ref={ref} style={{
          ...
          <div style={{ textAlign:"center", animation:"acReveal .7s ease both" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:14, marginBottom:12 }}>
              ...
              <span style={{ fontSize:11, color:tk.pageEyebrow, letterSpacing:".36em", fontFamily:"'Space Mono',monospace", transition:"color .5s" }}>✦ 1DEV CODEX ✦</span>
              <div style={{ height:1, width:52, background:`linear-gradient(90deg,${tk.pageEyebrowLine},transparent)` }} />
            </div>
            <h1 style={{
              fontFamily: tk.pageHeadingFont,
              ...
              color: tk.pageHeadingColor,
              textShadow: tk.pageHeadingShadow,
              lineHeight:1.1, marginBottom:8,
              transition:"color .5s,text-shadow .5s",
            }}>AWARD & CERTIFICATE</h1>
            <div style={{ height:1, maxWidth:200, margin:"10px auto 0", background:`linear-gradient(90deg,transparent,${tk.pageEyebrowLine},transparent)` }} />
          </div>
