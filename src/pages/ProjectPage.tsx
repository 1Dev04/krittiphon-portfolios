...
interface PTheme {
  ...
  cardTopLine: (c: string) => string;
  cardImgFade: string;
  cardHeading: string;
  cardSubText: string;
  btnDetail: (c: string) => string;
  btnFlip: (c: string) => string;
  // card back
  cardBackBg: string;
  ...
  arrowBgHov: string;
  arrowBorder: string;
  arrowBorderHov: string;
  arrowColor: string;
  arrowColorHov: string;
  arrowShadow: string;
  arrowShadowHov: string;
  // lang toggle
  toggleBg: string;
  toggleBorder: string;
...
const SUN_P: PTheme = {
  ...
  titleGrad: "linear-gradient(135deg,#1a3d28 0%,#2d6a4f 45%,#52b788 100%)",
  titleDivider: "rgba(45,106,79,0.35)",
  titleSub: "rgba(20,50,30,0.35)",
  // card front
  cardFrontBg: "rgba(255,255,255,0.82)",
  ...
  cardHeading: "#1a3d28",
  cardSubText: "rgba(20,50,30,0.42)",
  btnDetail: (c) => `border:1px solid ${c}66;background:linear-gradient(135deg,${c}22,${c}0a);color:${c};`,
  btnFlip: (c) => `border:1px solid ${c}44;background:${c}14;color:${c};`,
  // card back
  ...
  cardBackGlow: (c) => `radial-gradient(ellipse 65% 45% at 50% 0%,${c}10 0%,transparent 70%)`,
  dotInactive: "rgba(45,106,79,0.2)",
  // arrows
  arrowBg: "rgba(255,255,255,0.88)",
  arrowBgHov: "rgba(45,106,79,0.12)",
  arrowBorder: "rgba(45,106,79,0.22)",
  arrowBorderHov: "rgba(45,106,79,0.55)",
  arrowColor: "rgba(45,106,79,0.75)",
  arrowColorHov: "#1a3d28",
  arrowShadow: "0 0 16px rgba(45,106,79,0.12)",
  arrowShadowHov: "0 0 28px rgba(45,106,79,0.22)",
  // lang toggle
  toggleBg: "rgba(255,255,255,0.70)",
  toggleBorder: "rgba(45,106,79,0.20)",
  toggleActiveBg: "linear-gradient(135deg,rgba(45,106,79,0.22),rgba(82,183,136,0.12))",
  toggleActiveBorder: "rgba(45,106,79,0.45)",
  toggleActiveColor: "#1a3d28",
  toggleColor: "rgba(20,50,30,0.28)",
  // modal
  ...
  modalTitle: (c) => `linear-gradient(135deg,#1a3d28,${c} 60%)`,
  modalText: "rgba(20,50,30,0.48)",
...
const MOON_P: PTheme = {
  ...
  titleGrad: "linear-gradient(135deg,#f1f5f9 0%,#a78bfa 35%,#67e8f9 70%,#f1f5f9 100%)",
  titleDivider: "rgba(167,139,250,0.35)",
  titleSub: "rgba(255,255,255,0.20)",
  cardFrontBg: "linear-gradient(165deg,rgba(255,255,255,.055),rgba(255,255,255,.015))",
  cardFrontBorder: (c) => `${c}28`,
  cardFrontShadow: "0 8px 40px rgba(0,0,0,.55),inset 0 1px 0 rgba(255,255,255,.07)",
  cardTopLine: (c) => `linear-gradient(90deg,transparent,${c}bb,transparent)`,
  cardImgFade: "linear-gradient(to top,rgba(5,5,14,.97) 0%,rgba(5,5,14,.1) 55%,transparent 100%)",
  cardHeading: "#e2e8f0",
  cardSubText: "rgba(255,255,255,.32)",
  btnDetail: (c) => `border:1px solid ${c}55;background:linear-gradient(135deg,${c}22,${c}0a);color:${c};`,
  btnFlip: (c) => `border:1px solid ${c}44;background:${c}14;color:${c};`,
  ...
  cardBackGlow: (c) => `radial-gradient(ellipse 65% 45% at 50% 0%,${c}0d 0%,transparent 70%)`,
  dotInactive: "rgba(167,139,250,.2)",
  arrowBg: "rgba(8,8,24,.88)",
  arrowBgHov: "rgba(167,139,250,.2)",
  ...
  arrowShadowHov: "0 0 28px rgba(124,58,237,.4)",
  toggleBg: "rgba(255,255,255,.04)",
  toggleBorder: "rgba(167,139,250,.2)",
  toggleActiveBg: "linear-gradient(135deg,rgba(167,139,250,.28),rgba(103,232,249,.14))",
  toggleActiveBorder: "rgba(167,139,250,.45)",
  toggleActiveColor: "#a78bfa",
  toggleColor: "rgba(255,255,255,.28)",
  ...
  modalTitle: (c) => `linear-gradient(135deg,#f1f5f9,${c} 60%)`,
...
function ActionBtn({ href, color, label, icon }: { href: string; color: string; label: string; icon: React.ReactNode }) {
  ...
  return (
    <a href={href} target="_blank" rel="noopener noreferrer"
      ...
      style={{
        display: "flex", alignItems: "center", gap: 5, padding: "5px 14px", borderRadius: 999,
        border: `1px solid ${h ? color + "99" : color + "33"}`, background: h ? `${color}1c` : "transparent",
        color: h ? color : color + "66", textDecoration: "none", fontSize: 10, fontWeight: 700,
        fontFamily: "'Space Mono',monospace", letterSpacing: ".06em",
        boxShadow: h ? `0 0 12px ${color}33` : "none",
        transform: h ? "translateY(-1px)" : "none",
        transition: "all .25s cubic-bezier(.22,.68,0,1.2)", whiteSpace: "nowrap"
      }}>
      {icon}{label}
    </a>
  );
...
function FlipCard({ item, lang, index, onOpen, isDragging, tk }:
  {
    item: ProjectItem; lang: Lang; index: number; onOpen: (i: ProjectItem) => void;
    isDragging: React.MutableRefObject<boolean>; tk: PTheme
  }) {
    ...
    >
      <div style={{
        ...
        <div style={{
          ...
          overflow: "hidden", background: tk.cardFrontBg,
          border: `1px solid ${tk.cardFrontBorder(c)}`,
          backdropFilter: "blur(16px)", boxShadow: tk.cardFrontShadow,
          display: "flex", flexDirection: "column", transition: "background .5s,border-color .5s"
        }}>
          <div style={{
            position: "absolute", top: 0, left: "8%", right: "8%", height: 1.5,
            background: tk.cardTopLine(c), borderRadius: 2
          }} />
          <div style={{ height: 185, overflow: "hidden", flexShrink: 0, background: "#080818", position: "relative" }}>
            ...
            <div style={{
              ...
              background: "rgba(0,0,0,.6)", border: `1px solid ${c}33`,
              display: "flex", alignItems: "center", justifyContent: "center",
              fontSize: 13, color: `${c}88`, fontFamily: "serif"
            }}>{RUNES[index % 10]}</div>
            <div style={{
              position: "absolute", top: 10, right: 10, background: `${c}18`,
              border: `1px solid ${c}44`, borderRadius: 999, padding: "3px 9px",
              fontSize: 9, color: c, fontFamily: "'Space Mono',monospace", backdropFilter: "blur(8px)"
            }}>
              {gl(item.year, lang)}</div>
          </div>
          <div style={{ padding: "14px 16px 16px", flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
            <h2 style={{
              margin: 0, fontSize: 12.5, fontWeight: 700, color: tk.cardHeading,
              fontFamily: "'Space Mono',monospace", lineHeight: 1.45,
              display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden",
              transition: "color .5s"
            }}>{gl(item.title, lang)}</h2>
            <div style={{ height: 1, background: `linear-gradient(90deg,${c}55,transparent)` }} />
            <p style={{
              margin: 0, fontSize: 10, color: tk.cardSubText, fontFamily: "'Space Mono',monospace",
              lineHeight: 1.65, flex: 1, transition: "color .5s"
            }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <button onClick={e => { e.stopPropagation(); onOpen(item); }}
                style={{
                  ...
                  border: `1px solid ${c}55`, background: `linear-gradient(135deg,${c}22,${c}0a)`,
                  color: c, fontSize: 9, fontFamily: "'Space Mono',monospace", fontWeight: 700,
                  letterSpacing: ".08em", boxShadow: `0 0 14px ${c}22`
                }}>⊕ DETAIL</button>
              <button onClick={e => { e.stopPropagation(); setFlipped(f => !f); }}
                style={{
                  flex: 1, padding: "6px 0", borderRadius: 999, cursor: "pointer",
                  border: `1px solid ${c}44`, background: `${c}14`,
                  color: c, fontSize: 9, fontFamily: "'Space Mono',monospace",
        ...
        <div style={{
          position: "absolute", inset: 0, backfaceVisibility: "hidden", borderRadius: 20,
          transform: "rotateY(180deg)", overflow: "hidden", background: tk.cardBackBg,
          border: `1px solid ${tk.cardBackBorder(c)}`, backdropFilter: "blur(20px)",
          boxShadow: tk.cardBackShadow(c),
          display: "flex", flexDirection: "column", padding: "18px 16px 16px", gap: 9,
          transition: "background .5s,border-color .5s"
        }}>
          <div style={{
            position: "absolute", top: 0, left: "8%", right: "8%", height: 2,
            background: `linear-gradient(90deg,transparent,${c},transparent)`, opacity: .8, borderRadius: 2
          }} />
          <div style={{ position: "absolute", inset: 0, background: tk.cardBackGlow(c), pointerEvents: "none" }} />
          <div style={{
            fontSize: 26, color: `${c}18`, fontFamily: "serif", textAlign: "right",
            position: "absolute", top: 12, right: 14, lineHeight: 1
          }}>{RUNES[index % 10]}</div>
          <div style={{ paddingRight: 24 }}>
            <h3 style={{
              margin: 0, fontSize: 12, fontWeight: 700, color: c,
              fontFamily: "'Space Mono',monospace", lineHeight: 1.4,
              textShadow: `0 0 14px ${c}88`,
              display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden"
            }}>
              {gl(item.title, lang)}</h3>
            <span style={{
              fontSize: 8.5, color: `${c}77`, letterSpacing: ".1em",
              fontFamily: "'Space Mono',monospace"
            }}>{gl(item.year, lang)}</span>
          </div>
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 5, flex: 1, overflow: "hidden" }}>
            {item.head.slice(0, 4).map((h, i) => (
              <li key={i} style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                <span style={{
                  fontSize: 9.5, fontWeight: 700, color: c, fontFamily: "'Space Mono',monospace",
                  display: "flex", alignItems: "center", gap: 4
                }}>
                  <span style={{ fontSize: 6, opacity: .55 }}>◆</span>{gl(h, lang)}
                </span>
                {item.desc[i] && <span style={{
                  fontSize: 9.5, color: tk.cardSubText,
                  fontFamily: "'Space Mono',monospace", lineHeight: 1.55, paddingLeft: 11,
                  display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden",
                  transition: "color .5s"
                }}>{gl(item.desc[i], lang)}</span>}
              </li>
            ))}
          </ul>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 2 }}>
            <button onClick={e => { e.stopPropagation(); setFlipped(false); }}
              style={{
                padding: "5px 0", borderRadius: 999, border: `1px solid ${c}33`,
                background: "transparent", color: `${c}77`, fontSize: 9,
                fontFamily: "'Space Mono',monospace", fontWeight: 700, cursor: "pointer",
                letterSpacing: ".08em", flex: 1
              }}>↩ BACK</button>
            <button onClick={e => { e.stopPropagation(); onOpen(item); }}
              style={{
                flex: 2, padding: "5px 0", borderRadius: 999, border: `1px solid ${c}55`,
                background: `linear-gradient(135deg,${c}22,${c}0a)`, color: c, fontSize: 9,
                fontFamily: "'Space Mono',monospace", fontWeight: 700, cursor: "pointer",
                letterSpacing: ".08em", boxShadow: `0 0 14px ${c}22`
              }}>⊕ DETAIL</button>
          </div>
...
function Modal({ item, lang, onClose, tk }: { item: ProjectItem; lang: Lang; onClose: () => void; tk: PTheme }) {
  const c = item.color || "#a78bfa";
  useEffect(() => {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      background: "rgba(2,2,12,.9)", backdropFilter: "blur(20px)",
      display: "flex", alignItems: "center", justifyContent: "center",
      padding: "24px 16px", animation: "mBg .28s ease both"
    }}>
      <div onClick={e => e.stopPropagation()} style={{
        width: "100%", maxWidth: 700, maxHeight: "88vh",
        borderRadius: 24, background: tk.modalBg,
        border: `1px solid ${tk.modalBorder(c)}`,
        boxShadow: tk.modalShadow(c),
        overflow: "hidden", display: "flex", flexDirection: "column",
        animation: "mIn .42s cubic-bezier(.22,.68,0,1.2) both", position: "relative",
        transition: "background .5s"
      }}>
        <div style={{
          position: "absolute", top: 0, left: "5%", right: "5%", height: 2,
          background: `linear-gradient(90deg,transparent,${c},transparent)`, borderRadius: 2
        }} />
        <div style={{
          position: "absolute", top: 0, left: 0, right: 0, height: 280,
          background: `radial-gradient(ellipse 70% 55% at 50% 0%,${c}11 0%,transparent 70%)`,
          pointerEvents: "none"
        }} />
        <div style={{ height: 215, overflow: "hidden", flexShrink: 0, background: "#060612", position: "relative" }}>
          ...
          <button onClick={onClose} style={{
            ...
            borderRadius: 10, background: "rgba(0,0,0,.65)", border: `1px solid ${c}33`,
            color: "rgba(255,255,255,.65)", cursor: "pointer",
            display: "flex", alignItems: "center", justifyContent: "center"
          }}><IcoX /></button>
          <div style={{ position: "absolute", bottom: 14, left: 20, display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{
              width: 32, height: 32, borderRadius: 10, background: `${c}22`,
              border: `1px solid ${c}44`, display: "flex", alignItems: "center", justifyContent: "center",
              fontSize: 16, color: `${c}99`, fontFamily: "serif"
            }}>{RUNES[projects.indexOf(item) % 10]}</div>
            <div style={{
              background: `${c}1a`, border: `1px solid ${c}44`, borderRadius: 999,
              padding: "4px 12px", fontSize: 10, color: c, fontFamily: "'Space Mono',monospace",
        ...
        <div style={{ padding: "22px 26px 28px", overflow: "auto", display: "flex", flexDirection: "column", gap: 14 }}>
          <h2 style={{
            ...
            background: tk.modalTitle(c),
            WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent",
            backgroundClip: "text", lineHeight: 1.3, letterSpacing: ".03em"
          }}>
            {gl(item.title, lang)}</h2>
              ...
              {item.head.map((h, i) => (
                <li key={i} style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                  <span style={{
                    ...
                    fontFamily: "'Space Mono',monospace", letterSpacing: ".07em",
                    display: "flex", alignItems: "center", gap: 6
                  }}>
                    <span style={{ fontSize: 7, opacity: .65 }}>◆</span>{gl(h, lang)}
                  </span>
                  {item.desc[i] && <p style={{
                    margin: 0, fontSize: 12, color: tk.modalText,
                    fontFamily: "'Space Mono',monospace", lineHeight: 1.8, paddingLeft: 14,
                    transition: "color .5s"
                  }}>{gl(item.desc[i], lang)}</p>}
                </li>
              ))}
            </ul>
          ) : (
            <p style={{
              margin: 0, fontSize: 12.5, color: tk.modalText,
...
function ArrowBtn({ dir, onClick, tk }: { dir: "left" | "right"; onClick: () => void; tk: PTheme }) {
  const [h, sh] = useState(false);
  return (
    <button onClick={onClick} onMouseEnter={() => sh(true)} onMouseLeave={() => sh(false)}
      style={{
        position: "absolute", [dir === "left" ? "left" : "right"]: 14, top: "50%",
        transform: h ? "translateY(-50%) scale(1.1)" : "translateY(-50%)", zIndex: 5,
        width: 44, height: 44, borderRadius: 13,
        background: h ? tk.arrowBgHov : tk.arrowBg,
        border: `1px solid ${h ? tk.arrowBorderHov : tk.arrowBorder}`,
        color: h ? tk.arrowColorHov : tk.arrowColor,
        cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
        backdropFilter: "blur(14px)",
        boxShadow: h ? tk.arrowShadowHov : tk.arrowShadow,
...
const ProjectPage: React.FC = () => {
  ...
  const onPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (hasDragged.current) { el.scrollLeft = dragStartSL.current + dx; updateArrows(); }
  };
    ...
    <>
      ...
      <section id="projects-reference" style={{
        ...
        <div style={{
          ...
          backgroundSize: "64px 64px", transition: "background-image 0.5s"
        }} />

        {/* orbs */}
        <div style={{
          position: "absolute", width: 600, height: 600, top: "-20%", right: "-10%",
          borderRadius: "50%", filter: "blur(90px)", background: tk.orbA,
          animation: "orbF 16s ease-in-out infinite", pointerEvents: "none", transition: "background 0.5s"
        }} />
        <div style={{
          position: "absolute", width: 500, height: 500, bottom: "-5%", left: "-12%",
          borderRadius: "50%", filter: "blur(80px)", background: tk.orbB,
          animation: "orbF 20s ease-in-out 5s infinite", pointerEvents: "none", transition: "background 0.5s"
        }} />
        ...
        {tk.showDots && Array.from({ length: 14 }, (_, i) => (
          <div key={i} style={{
            ...
            animation: `orbF ${5 + i % 6}s ease-in-out ${(i * .3) % 5}s infinite`,
            pointerEvents: "none"
          }} />
        ))}
        ...
        <div ref={titleRef} style={{
          textAlign: "center", marginBottom: 44, padding: "0 8%",
          position: "relative", zIndex: 3,
          opacity: titleVis ? 1 : 0, transform: titleVis ? "none" : "translateY(24px)",
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, marginBottom: 10 }}>
            ...
            <span style={{
              fontSize: 10, color: tk.eyebrow, letterSpacing: ".32em",
          ...
          <div style={{
            margin: "12px auto 0", maxWidth: 280, height: 1,
            background: `linear-gradient(90deg,transparent,${tk.titleDivider},transparent)`,
            transition: "background 0.5s"
          }} />
          <p style={{
            marginTop: 10, fontSize: 10, color: tk.titleSub, letterSpacing: ".18em",
            transition: "color 0.5s"
          }}>
            {projects.length}&nbsp; Crafted Spells &nbsp;·&nbsp; Drag &nbsp;·&nbsp; Flip &nbsp;·&nbsp; Click to expand
          </p>
        </div>
        ...
        <div style={{ position: "relative", zIndex: 3 }}>
          <div style={{
            ...
            background: tk.fadeLeft, pointerEvents: "none", zIndex: 4, transition: "background 0.5s"
          }} />
          <div style={{
            position: "absolute", right: 0, top: 0, bottom: 0, width: 72,
            background: tk.fadeRight, pointerEvents: "none", zIndex: 4, transition: "background 0.5s"
          }} />
          {canL && <ArrowBtn dir="left" onClick={() => scrollTo(activeIdx - 1)} tk={tk} />}
          {canR && <ArrowBtn dir="right" onClick={() => scrollTo(activeIdx + 1)} tk={tk} />}

          <div ref={scrollRef} className="ps-hide"
            ...
            style={{
              display: "flex", gap: 20, overflowX: "auto", padding: "20px 8% 28px",
              cursor: grabbing ? "grabbing" : "grab", userSelect: "none",
              opacity: switching ? 0 : 1, transition: "opacity .24s"
            }}>
            {projects.map((item, i) => (
              <div key={i} style={{ animation: `fadeUp .55s ease ${i * .04}s both`, flexShrink: 0 }}>
                <FlipCard item={item} lang={lang} index={i} onOpen={setModal} isDragging={isDragging} tk={tk} />
              </div>
            ))}
          </div>
        ...
        <div style={{ display: "flex", justifyContent: "center", marginTop: 32, position: "relative", zIndex: 3 }}>
          <div style={{
            ...
            borderRadius: 999, padding: 4, backdropFilter: "blur(14px)",
            transition: "background 0.5s,border-color 0.5s"
          }}>
            {(["th", "en"] as Lang[]).map(l => (
              <button key={l} onClick={() => switchLang(l)} style={{
                padding: "7px 26px", borderRadius: 999,
                border: lang === l ? `1px solid ${tk.toggleActiveBorder}` : "1px solid transparent",
                background: lang === l ? tk.toggleActiveBg : "transparent",
                color: lang === l ? tk.toggleActiveColor : tk.toggleColor,
        ...
        <div style={{
          ...
          background: tk.bottomFade, pointerEvents: "none", transition: "background 0.5s"
        }} />
      </section>

      {modal && <Modal item={modal} lang={lang} onClose={() => setModal(null)} tk={tk} />}
    </>
  );
};
