...
// Icons
// ─────────────────────────────────────────────
const IcoLink = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" width="13" height="13"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71" /><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71" /></svg>;
const IcoGH   = () => <svg viewBox="0 0 24 24" fill="currentColor" width="13" height="13"><path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" /></svg>;
const IcoYT   = () => <svg viewBox="0 0 24 24" fill="currentColor" width="13" height="13"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" /></svg>;
const IcoX    = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="16" height="16"><path d="M18 6L6 18M6 6l12 12" /></svg>;
const IcoLeft = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="17" height="17"><path d="M15 18l-6-6 6-6" /></svg>;
const IcoRight= () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="17" height="17"><path d="M9 18l6-6-6-6" /></svg>;
...
function FlipCard({ item, lang, index, onOpen, isDragging, tk }: {
  item: ProjectItem; lang: Lang; index: number;
  onOpen: (i: ProjectItem) => void;
  ...
  const btnBase = (accent: string, variant: "solid" | "ghost") => ({
    ...
    letterSpacing: ".08em",
    border: variant === "solid" ? `1px solid ${accent}55` : `1px solid ${accent}33`,
    background: variant === "solid"
      ? `linear-gradient(135deg,${accent}28,${accent}10)`
      : "transparent",
    color: variant === "solid" ? accent : `${accent}77`,
    boxShadow: variant === "solid" ? `0 0 14px ${accent}22` : "none",
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ padding:"14px 16px 16px", flex:1, display:"flex", flexDirection:"column", gap:8 }}>
            ...
            <p style={{ margin:0, fontSize:10, color:tk.cardSubText, fontFamily:isTH?"'Sarabun',sans-serif":"'Space Mono',monospace", lineHeight:1.65, flex:1, transition:"color .5s" }}>
              {item.head.length > 0
                ? item.head.slice(0,3).map(h => gl(h,lang)).join(" · ") + (item.head.length > 3 ? "…" : "")
                : "Click to explore →"}
            </p>
        ...
        <div style={{
          ...
          border:`1px solid ${tk.cardBackBorder(c)}`,
          backdropFilter:"blur(20px)", boxShadow:tk.cardBackShadow(c),
          ...
          <div style={{ paddingRight:24 }}>
            ...
            <span style={{ fontSize:8.5, color:`${c}77`, letterSpacing:".1em", fontFamily:"'Space Mono',monospace" }}>
              {gl(item.year, lang)}
            </span>
          </div>
          ...
          <ul style={{ margin:0, padding:0, listStyle:"none", display:"flex", flexDirection:"column", gap:5, flex:1, overflow:"hidden" }}>
            {item.head.slice(0,4).map((h,i) => (
              <li key={i} style={{ display:"flex", flexDirection:"column", gap:1 }}>
                ...
                {item.desc[i] && <span style={{ fontSize:9.5, color:tk.cardBackSubText, fontFamily:isTH?"'Sarabun',sans-serif":"'Space Mono',monospace", lineHeight:1.55, paddingLeft:11, display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical", overflow:"hidden", transition:"color .5s" }}>
                  {gl(item.desc[i],lang)}
                </span>}
              </li>
            ))}
    ...
    <div onClick={onClose} style={{ position:"fixed", inset:0, zIndex:9999, background:"rgba(2,2,12,.90)", backdropFilter:"blur(20px)", display:"flex", alignItems:"center", justifyContent:"center", padding:"24px 16px", animation:"mBg .28s ease both" }}>
      <div onClick={e => e.stopPropagation()} style={{ width:"100%", maxWidth:700, maxHeight:"88vh", borderRadius:24, background:tk.modalBg, border:`1px solid ${tk.modalBorder(c)}`, boxShadow:tk.modalShadow(c), overflow:"hidden", display:"flex", flexDirection:"column", animation:"mIn .42s cubic-bezier(.22,.68,0,1.2) both", position:"relative", transition:"background .5s" }}>
        ...
        <div style={{ height:215, overflow:"hidden", flexShrink:0, background:"#060612", position:"relative" }}>
          ...
          <button onClick={onClose} style={{ position:"absolute", top:14, right:14, width:34, height:34, borderRadius:10, background:"rgba(0,0,0,.65)", border:`1px solid ${c}33`, color:"rgba(255,255,255,.65)", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
            <IcoX />
          </button>
          <div style={{ position:"absolute", bottom:14, left:20, display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:32, height:32, borderRadius:10, background:`${c}22`, border:`1px solid ${c}44`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, color:`${c}99`, fontFamily:"serif" }}>
              {RUNES[projects.indexOf(item) % 10]}
            </div>
            <div style={{ background:`${c}1a`, border:`1px solid ${c}44`, borderRadius:999, padding:"4px 12px", fontSize:10, color:c, fontFamily:"'Space Mono',monospace", backdropFilter:"blur(8px)" }}>
              {gl(item.year, lang)}
            </div>
          </div>
        </div>
        ...
        <div style={{ padding:"22px 26px 28px", overflow:"auto", display:"flex", flexDirection:"column", gap:14 }}>
          ...
          {item.head.length > 0 ? (
            <ul style={{ margin:0, padding:0, listStyle:"none", display:"flex", flexDirection:"column", gap:12 }}>
              {item.head.map((h,i) => (
                <li key={i} style={{ display:"flex", flexDirection:"column", gap:3 }}>
                  ...
                  {item.desc[i] && <p style={{ margin:0, fontSize:12, color:tk.modalText, fontFamily:isTH?"'Sarabun',sans-serif":"'Space Mono',monospace", lineHeight:1.8, paddingLeft:14, transition:"color .5s" }}>
                    {gl(item.desc[i],lang)}
                  </p>}
                </li>
              ))}
            ...
            <p style={{ margin:0, fontSize:12.5, color:tk.modalText, fontFamily:"'Space Mono',monospace", lineHeight:1.8, transition:"color .5s" }}>
...
const ProjectPage: React.FC = () => {
  ...
  const smoothScrollTo = (t: number) => scrollRef.current?.scrollTo({ left:t, behavior:"smooth" });
  const scrollTo = (i: number) => smoothScrollTo(Math.max(0, Math.min(projects.length-1, i)) * CARD_W);

  const onPointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    if ((e.target as HTMLElement).closest("button,a")) return;
    const el = scrollRef.current; if (!el) return;
    isDragging.current = false; hasDragged.current = false;
    dragStartX.current = e.clientX; dragStartSL.current = el.scrollLeft;
    el.setPointerCapture(e.pointerId); setGrab(true);
  };
  const onPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    const el = scrollRef.current; if (!el || !el.hasPointerCapture(e.pointerId)) return;
    const dx = dragStartX.current - e.clientX;
    if (!hasDragged.current && Math.abs(dx) > DRAG_THRESHOLD) { hasDragged.current = true; isDragging.current = true; }
  ...
  const onPointerUp = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (el) {
      ...
      if (hasDragged.current) smoothScrollTo(Math.round(el.scrollLeft / CARD_W) * CARD_W);
    }
    setGrab(false);
  ...
  const activeColor = projects[activeIdx]?.color || tk.eyebrow;
  ...
  return (
    <>
      ...
      <section id="projects-reference" style={{
        ...
        fontFamily:"'Space Mono',monospace", transition:"background .5s ease",
      }}>
        {/* Grid */}
        <div style={{ position:"absolute", inset:0, pointerEvents:"none", backgroundImage:`linear-gradient(${tk.gridLine} 1px,transparent 1px),linear-gradient(90deg,${tk.gridLine} 1px,transparent 1px)`, backgroundSize:"64px 64px" }} />
        ...
        <div ref={titleRef} style={{ textAlign:"center", marginBottom:44, padding:"0 8%", position:"relative", zIndex:3, opacity:titleVis?1:0, transform:titleVis?"none":"translateY(24px)", transition:"opacity .7s ease,transform .7s cubic-bezier(.22,.68,0,1.2)" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:12, marginBottom:10 }}>
            ...
            <span style={{ fontSize:10, color:tk.eyebrow, letterSpacing:".32em", textTransform:"uppercase", transition:"color .5s" }}>✦ 1DEV Codex ✦</span>
          ...
          <div style={{ margin:"12px auto 0", maxWidth:280, height:1, background:`linear-gradient(90deg,transparent,${tk.titleDivider},transparent)`, transition:"background .5s" }} />
          <p style={{ marginTop:10, fontSize:10, color:tk.titleSub, letterSpacing:".18em", transition:"color .5s" }}>
            {projects.length}&nbsp; Crafted Spells &nbsp;·&nbsp; Drag &nbsp;·&nbsp; Flip &nbsp;·&nbsp; Click
          </p>
        </div>
